...
from explain.algorithm.base import ExplainerAlgorithm
...
class GraphMaskExplainer(ExplainerAlgorithm):
    ...
    def forward(
        ...
        **kwargs,
        ...
        edge_mask, layer_edge_mask = self._explain(model, index=index)
        edge_mask = edge_mask[:edge_index.size(1)]
        for each in layer_edge_mask:
            each = each[:edge_index.size(1)]
        return Explanation(node_mask=node_mask, edge_mask=edge_mask,layer_edge_mask=layer_edge_mask)
